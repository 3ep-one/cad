...
def compile():
    subprocess.call(
        "iverilog -o test_bench_tb file_reader_a.v file_reader_b.v file_writer.v adder.v test_bench.v test_bench_tb.v",
        shell=True,
    )
...
def run_test(stimulus_a, stimulus_b):
    ...
    test = subprocess.Popen(
        "c_test/test", stdin=subprocess.PIPE, stdout=subprocess.PIPE
    )
    ...
    for expected, actual, a, b in zip(
        expected_responses, actual_responses, stimulus_a, stimulus_b
    ):
...
stimulus_a = [
    ...
    for i in permutations(
        [0x80000000, 0x00000000, 0x7F800000, 0xFF800000, 0x7FC00000, 0xFFC00000], 2
    )
]
stimulus_b = [
    i[1]
